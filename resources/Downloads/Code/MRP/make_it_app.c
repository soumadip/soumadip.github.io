...
int main()
{
	char cBuff[SIZE]="done!! ultimately...oww man!";
	socklen_t length;
		...
		P(unackSem);
			...
			while(ptr!=NULL)
			{
				...
				if(timevaldiff(&ptr->timestamp,&now)>=300)
				{
					...
					buf[0]='1';buf[1]=ptr->packet[0];buf[2]=ptr->packet[1];
					
					sendto(sockfd,buf,3,0,(struct sockaddr*)&sAddress,sizeof(sAddress));
					printf("Resend:: (%d)%c\n",buf[1],buf[2]);
...
void* receive_routine(void* arg)
{
	...
	while(1)
	{
		recvfrom(sockfd,pack,3, 0,(struct sockaddr *)&cAddress, &length);
					...
					while(ptr!=NULL)
					{
						if(ptr->packet[0]==pack[1])//&&!strcmp(ptr->addr,inet_ntoa(cAddress.sin_addr)))
						{
							if(!--ackCounter)
							{
								...
								pack[0]='1';pack[1]=51;pack[2]='\0';
								sendto(sockfd,pack,3,0,(struct sockaddr*)&cAddress,sizeof(cAddress));
							}
...
ssize_t r_sendto(int sockfd, const void *buf, size_t len, int flags,const struct sockaddr_in *dest_addr, socklen_t addrlen)
{
	...
	unackd *ptr;
		...
		for(i=0;i<len;i++)
		{
			...
			//usleep(115000);
			sendto(sockfd,temp,3,flags,(struct sockaddr*)dest_addr,addrlen);
