import java.util.*;
        class TaskAllotmentInfo
        {
            int taskNo;
            int taskAssigned;
        }
        class TaskRecord {
          TaskAllotmentInfo tai[] = new TaskAllotmentInfo[10];
          void enterInfo(int upperLimit, int arraySize) {
                     int tn;
                     int found = 0;
                     if(upperLimit <=0) {
                               throw new IllegalArgumentException("UpperLimit must bepve: " + upperLimit);
...
class Client implements Runnable{
          ...
          void getJobNo(int uLimit) {
                     if(uLimit <=0) {
                               throw new IllegalArgumentException("UpperLimit must bepositive: " + uLimit);
                     }
                     Random generator = new Random();
