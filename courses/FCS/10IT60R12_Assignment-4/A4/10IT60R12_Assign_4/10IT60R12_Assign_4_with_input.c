...
/* 
	...
	Input: node pointer
	Output: node pointer
	Description: this function gets the node pointer on which tree to be created by argument
			then takes no of nodes to insert
			then nodes itself(for any repeat, that data will be rejected without any notification)
*/
tree* createTree(tree *root)
{
	int no,data;
	scanf("%d",&no);			//input no of nodes
	while(no)
	{
		scanf("%d",&data);		//input data or node value
		root=insertNodeBST(root,data);	//calls insertNodeBST
		no--;
	}
	return root;
}

void makeNameAndDisplay(tree *root,int testcase,int seq)
{
	char filename[10]="tree_ _";
	
	filename[5]=(char)(testcase+48);
	filename[7]=(char)(seq+48);
	filename[8]='\0';
	
	printf("\n FILENAME:: %s\nFILE CONTENTS\n\n",filename);
	d(root);printf("\n");					//calls d
	display(root,filename);
}
...
int main()
{
	...
	while(testcase--)
	{
		tno++;root1=NULL;root2=NULL;
		root1=createTree(root1);		//calls createTree
		root2=createTree(root2);		//calls createTree

		makeNameAndDisplay(root1,tno,1);
		makeNameAndDisplay(root2,tno,2);
	
		root1=mergeBST(root1,root2);		//calls mergeBST
		makeNameAndDisplay(root1,tno,3);
	
		destroy(root1);				//calls destroy
		destroy(root2);				//calls destroy
	}	
